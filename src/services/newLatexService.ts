...
export const generateLatexPDF = async (data: AcademicResumeData, templateId: string): Promise<Blob> => {
  ...
  const htmlContent = convertLatexToHTML(data, templateId);
  ...
  return new Promise((resolve) => {
    printWindow.onload = () => {
      ...
      // Create a dummy blob (in real implementation, this would be the actual PDF)
...
const convertLatexToHTML = (data: AcademicResumeData, templateId: string): string => {
  const colors = {
  ...
  if (templateId === 'academic') {
    return generateAcademicHTML(data, templateColors);
  } else if (templateId === 'techmodern') {
    return generateTechModernHTML(data, templateColors);
  } else if (templateId === 'classicpro') {
    return generateClassicProHTML(data, templateColors);
  }
  
  return generateAcademicHTML(data, templateColors);
};

const generateAcademicHTML = (data: AcademicResumeData, colors: any): string => {
  return `<!DOCTYPE html>
<html>
<head>
  <meta charset="UTF-8">
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .container { width: 100%; max-width: 8.27in; margin: 0 auto; padding: 0; }
    .header { display: flex; justify-content: space-between; margin-bottom: 1em; }
    .header-left { flex: 0.64; }
    .header-right { flex: 0.34; text-align: right; }
    .name { font-size: 22pt; font-weight: bold; line-height: 1.1; margin-bottom: 0.5ex; }
    ...
    .education-table th { font-weight: bold; background: #f8f9fa; }
    .bold-item { margin-bottom: 1em; }
    .item-header { 
      display: flex; 
    ...
    .skills-list { margin-left: 1em; }
    .skill-item { margin-bottom: 0.4em; }
    @media print {
  ...
  <div class="container">
    ...
    <div class="header">
      <div class="header-left">
        ...
        ${data.personalInfo.address ? `<div><strong>${data.personalInfo.address}</strong></div>` : ''}
      </div>
      <div class="header-right contact-info">
        ${data.personalInfo.phone ? `<div><strong>${data.personalInfo.phone}</strong></div>` : ''}
    ...
    <div class="section">
      ...
      <div class="bold-item">
        <div class="item-header">
          ...
          <span><em>${exp.duration}</em></span>
        </div>
        <div class="sub-items">
          <div class="sub-item"><em>${exp.position}</em></div>
    ...
    <div class="section">
      ...
      <div class="bold-item">
        <div class="item-header">
          ...
          <span><em>${project.duration || ''}</em></span>
        </div>
        <div class="sub-items">
          ${project.description.map(desc => `<div class="sub-item">${desc}</div>`).join('')}
          ${project.githubLink ? `<div class="sub-item"><a href="${project.githubLink}" style="color: inherit;"><strong>GitHub</strong>: ${project.githubLink}</a></div>` : ''}
        </div>
      </div>
      `).join('')}
    </div>
    ...
    <div class="section">
      ...
      <div class="skills-list">
        ${data.skills.languages.length > 0 ? `<div class="skill-item"><strong>Languages:</strong> ${data.skills.languages.join(', ')}</div>` : ''}
        ${data.skills.frontend.length > 0 ? `<div class="skill-item"><strong>Frontend:</strong> ${data.skills.frontend.join(', ')}</div>` : ''}
        ${data.skills.backend.length > 0 ? `<div class="skill-item"><strong>Backend:</strong> ${data.skills.backend.join(', ')}</div>` : ''}
        ${data.skills.tools.length > 0 ? `<div class="skill-item"><strong>Tools:</strong> ${data.skills.tools.join(', ')}</div>` : ''}
        ${data.skills.databases && data.skills.databases.length > 0 ? `<div class="skill-item"><strong>Databases:</strong> ${data.skills.databases.join(', ')}</div>` : ''}
        ${data.skills.concepts && data.skills.concepts.length > 0 ? `<div class="skill-item"><strong>Concepts:</strong> ${data.skills.concepts.join(', ')}</div>` : ''}
      </div>
    </div>
    ...
    <div class="section">
      ...
      <div class="skills-list">
        ${data.codingProfiles.map(profile => `<div class="skill-item"><a href="${profile.url}" style="color: inherit;"><strong>${profile.platform}</strong>: ${profile.url}</a></div>`).join('')}
      </div>
    </div>
    ` : ''}
  </div>
...
const generateTechModernHTML = (data: AcademicResumeData, colors: any): string => {
  return `<!DOCTYPE html>
<html>
<head>
  <meta charset="UTF-8">
...
const generateClassicProHTML = (data: AcademicResumeData, colors: any): string => {
  return `<!DOCTYPE html>
<html>
<head>
  <meta charset="UTF-8">
